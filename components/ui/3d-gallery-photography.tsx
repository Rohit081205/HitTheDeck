...
function ReducedMotionGallery({ images }: { images: ImageItem[] }) {
	const [index, setIndex] = useState(0);
	const normalizedImages = useMemo(() => images.map((img) => typeof img === 'string' ? { src: img } : img), [images]);
	...
	return (
		<div className="relative w-full h-full bg-zinc-950 flex items-center justify-center overflow-hidden">
			<AnimatePresence mode="wait">
				...
				>
					<img 
						src={normalizedImages[index].src} 
						alt="Stadium" 
...
export default function InfiniteGallery({
	...
	style,
	fadeSettings = { fadeIn: { start: 0.05, end: 0.25 }, fadeOut: { start: 0.8, end: 0.95 } },
	blurSettings = { blurIn: { start: 0.0, end: 0.15 }, blurOut: { start: 0.8, end: 1.0 }, maxBlur: 6.0 },
	...
	if (prefersReducedMotion || !webglSupported) {
		return (
			<div className={className} style={style}>
				<ReducedMotionGallery images={images} />
			</div>
		);
	}
